 ...
 * @since 03/28/2022
 */
public class DeleteMemberNoteStep extends AbstractUIStep {

	private static final Logger logger = LoggerFactory.getLogger(DeleteMemberNoteStep.class);

	private final static String STEP_NAME = "DeleteMemberNote";
	private final static String STEP_DESC = "Delete member note";
	 ...
	 * Constructor
	 * 
	 * @param uniqueNoteIdentifier
	 * @param driver
	 */
	public DeleteMemberNoteStep(String uniqueNoteIdentifier, WebDriver driver) {
	...
	public void run() 
	{
		...
		try
		{
			...
			{
				memberNotesPO.deleteNote(uniqueNoteIdentifier);
			}
			else
			{
				memberNotesPO.deleteNote();
			}
